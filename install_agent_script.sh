...
supported_platform() {
  ...
  t_supported_platform_regex='(el-(5|6|7)-(i386|x86_64|s390x))|(debian-(6|7|8)-(i386|amd64))|(ubuntu-(14\.04|15\.04|15\.10|16\.04)-(i386|amd64))|(sles-(11|12)-(i386|x86_64|s390x))|(fedora-(2[4-5])-(i386|x86_64))|(solaris-(10|11)-(i386|sparc))|(aix-(5\.3|6\.1|7\.1)-power)|(osx-10\.(10|11|12)-x86_64)'
...
run_agent_install_from_url() {
    ...
    The required agent packages should be retrieved when puppet runs on the master, after which you can run the install.bash script again."
            ...
            fail "This method of agent installation is not supported for ${PLATFORM_TAG?} in Puppet Enterprise v2017.2.2"
        fi
    fi
...
if [ -z "${PLATFORM_NAME:-""}" ] || [ -z "${PLATFORM_RELEASE:-""}" ]; then
    ...
    # Test for Debian releases
        ...
        if grep -qi 'amazon linux' /etc/system-release; then
            ...
            fail "$(cat /etc/system-release) is not a supported platform for Puppet Enterprise v2017.2.2
        ...
        fail "$(uname -s) is not a supported platform for Puppet Enterprise v2017.2.2
            Please visit http://links.puppetlabs.com/puppet_enterprise_${PE_LINK_VER?}_platform_support to request support for this platform."
    fi
fi
...
run_agent_install_from_url "https://learning.puppetlabs.vm:8140/packages/2017.2.2/${PLATFORM_TAG}.bash" "$@"
